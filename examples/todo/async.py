...
app.config["SQLALCHEMY_DATABASE_URI"] = "sqlite+aiosqlite:///todo.sqlite"
...
async def show_all():
    ...
    todos = await db.session.execute(select)
    return await render_template("show_all.html", todos=todos.scalars().all())
...
async def update_done():
    ...
    for todo in (await db.session.execute(db.select(Todo))).scalars():
